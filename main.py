...
def extract_all_urls(url_map: Dict[str, Any]) -> List[str]:
    if isinstance(url_map, dict) and 'urls_by_depth' in url_map:
        return [u for _, urls in url_map['urls_by_depth'].items() for u in urls]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--url-map', required=True, help='Path to URL mapping JSON file')
...
def main() -> None:
    ...
    result = process_mapped_urls(args.url_map, args.max_depth, args.batch_size, args.limit)
